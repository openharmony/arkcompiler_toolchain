...
#include "wsservercontinuerunserver_fuzzer.h"
...
namespace OHOS {
    ...
    void WsServerContinueRunServerFuzzTest([[maybe_unused]]const uint8_t* data, size_t size)
    {
        if (size <= 0) {
            return;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::WsServerContinueRunServerFuzzTest(data, size);
    return 0;
}