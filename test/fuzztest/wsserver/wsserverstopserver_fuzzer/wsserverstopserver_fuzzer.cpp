...
#include "wsserverstopserver_fuzzer.h"
...
namespace OHOS {
    ...
    void WsServerStopServerFuzzTest([[maybe_unused]]const uint8_t* data, size_t size)
    {
        if (size <= 0) {
            return;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::WsServerStopServerFuzzTest(data, size);
    return 0;
}